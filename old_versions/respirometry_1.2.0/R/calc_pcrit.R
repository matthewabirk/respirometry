pcrit_internal = function(po2, mo2, level = 0.95, iqr = 1.5, NLR_m = 0.065, MR = NULL, mo2_threshold = Inf){
	...
	alpha = calc_alpha(po2 = po2, mo2 = mo2, mo2_threshold = mo2_threshold, MR = ifelse(is.null(MR), MR_mean_reg, MR))
...
calc_pcrit = function(po2, mo2, level = 0.95, iqr = 1.5, NLR_m = 0.065, MR = NULL, mo2_threshold = Inf, return_models = FALSE){
	l = pcrit_internal(po2, mo2, level, iqr, NLR_m, MR = MR, mo2_threshold = mo2_threshold)
...
#'
#' @return A base graphic plot is created. The alpha, breakpoint, LLO, NLR, and sub-PI,  Pcrit values are shown in the title. The broken-stick regression is shown by black lines. The black inverted triangle represents the breakpoint Pcrit. The dashed red curves signify the prediction interval used for the sub-PI Pcrit metric. Black circles represent oxyregulating observations used in the generation of the prediction interval, while transparent circles represent both the oxyconforming observations and those observations outside the IQR threshold (defined by \code{iqr}). The gray bands represent the confidence interval (defaults to 95\% but will change with \code{level}). The green curve represents the best fitting NLR function and the green inverted triangle represents the NLR Pcrit (modified by \code{NLR_m}) The blue line represents alpha, which was fit based on the blue circle observation. The blue inverted triangle represents the Pcrit-alpha at \code{MR}. The purple inverted triangle represents the Pcrit-LLO at \code{MR}.
#' 
...
plot_pcrit = function(po2, mo2, level = 0.95, iqr = 1.5, NLR_m = 0.065, MR = NULL, mo2_threshold = Inf, showNLRs = FALSE, ...){
	tryCatch({
		l = pcrit_internal(po2, mo2, level, iqr, NLR_m, MR = MR, mo2_threshold = mo2_threshold)
		inter1 = level
		plot_pcrit_internal = function(l, level = inter1, main = 
							paste0('Alpha @ MR of ', round(ifelse(is.null(MR), l$MR_mean_reg, l$MR), 2),' = ', round(l$pcrit_alpha, 3), 
							'\nBreakpoint = ', round(l$breakpoint, 3),
							'\nLLO @ MR of ', ifelse(is.null(MR), NA, round(l$MR, 2)),' = ', round(l$pcrit_LLO, 3), 
							'\nNLR (', l$best_mod, ') = ', round(ifelse(length(l$best_mod) > 0, l$nlr_pcrits[[l$best_mod]], NA), 3), 
							'\nSub-PI = ', round(l$sub_PI, 3)), shade = TRUE, ...){
		...
		graphics::points(x = l$alpha_obs[1], y = l$alpha_obs[2], col = 'blue', pch = 16)
