...
#' 
#' @description For most organisms, metabolic rate does not scale linearly, but rather according to a power function. This function estimates MO2 or size of an individual organism given the MO2 and size of another individual of a different size.
#' 
#' @details
#' \deqn{(MO2 = b0 * M^b)}
#' where \code{b0} is species-specific normalization constant, \code{M} is mass and \code{b} is the scaling coefficient which is 0.75 for many organisms.
#' 
#' For scaling of \strong{mass-specific} metabolic rates, use \code{b = -0.25} rather than \code{b = 0.75}.
#' 
