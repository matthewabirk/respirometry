#' Import data from a Loligo Systems Witrox transmitter
#'
...
#' @seealso \code{\link{import_presens}}, \code{\link{conv_o2}}
#' @examples
#' \dontrun{
...
import_witrox = function(file, o2_unit = 'percent_a.s.', date = '%m/%d/%Y %I:%M:%S %p', overwrite_sal = NULL, drop_channels = TRUE, split_channels = FALSE){
	...
	if(drop_channels == TRUE){
		...
		if(any(blank_cols)){
			...
			f = f[, rowSums(sapply(drop, function(i) grepl(i, colnames(f)))) == 0]
		}
	}
	if(split_channels == TRUE){
		...
		f = lapply(0:(sum(grepl('^CH_', colnames(f))) / 4 - 1), function(i){
			...
			if(any(!is.na(f_sub$O2))) f_sub = f_sub[birk::range_seq(which(is.na(f_sub$O2) == FALSE)), ]
