#' @title Calculate respirometer volume equivalent when there is airspace or bubbles
#' 
...
#' @seealso \code{\link{MO2}}, \code{\link{marelac::molvol}}
#' 
#' @examples
...
correct_bubble = function(resp_vol, bubble_vol, temp = 25, sal = 35, atm_pres = 1013.25){
	...
	bubble_vol = birk::conv_unit(bubble_vol, 'ml', 'l')
	vol_water = resp_vol - bubble_vol
	mol_o2 = bubble_vol * marelac::atmComp(species = 'O2') * marelac::molvol(t = temp, P = air_pres, species = 'O2')
	water_equiv = birk::conv_unit(mol_o2, 'mol', 'umol') / gas_satconc(S = sal, t = temp, P = air_pres, species = 'O2')
