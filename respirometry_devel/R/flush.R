...
#' flush(vol = 90, flow_rate = 2, duration = 20)
#' 
#' # Would it be worth it to extend the flush another five minutes? How much would that improve the exchange?
#' flush(vol = 90, flow_rate = 2, duration = 20, plot = TRUE) # another five minutes would increase exchange by nearly 10%. Perhaps that's worth the extra time...
#' 
#' @encoding UTF-8
#' @export

flush = function(vol, flow_rate, duration, perc_fresh, plot = FALSE){
  ...
  if(missing(perc_fresh)){
	...
	if(plot == TRUE){
		...
		plot(FR_range, perc_range, type = 'l', xlab = 'Flow rate (LPM)', ylab = 'Proportion exchanged')
		abline(v = flow_rate, lty = 2)
		
		dur_range = seq(duration - duration / 2, duration + duration / 2, length.out = 100)
		D = flow_rate / vol
		perc_range = 1 - exp(-D * dur_range)
		plot(dur_range, perc_range, type = 'l', xlab = 'Duration (min)', ylab = 'Proportion exchanged')
		abline(v = duration, lty = 2)
		layout(1)
	}
	return(result)
}